...
export function _shareFriendAndCommunity(apiRouter: string, routing: string, dispatch: any, ShareImage: string, activityId: string, OpenId: string, CustomerId: string, shareType1: string, shareType2: string) {
  ...
  wx.onMenuShareAppMessage({
    title: '分享好友', // 分享标题
    desc: '抽奖活动分享', // 分享描述
    link: `${BASE_URL}/WestLuckyDraw/ExternalShareGuide?address=dist&routing=${routing}&activityId=${activityId}&fromOpenid=${OpenId}&fromCustId=${CustomerId}&shareType=${shareType1}`, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
  ...
  wx.onMenuShareTimeline({
    title: '分享朋友圈', // 分享标题
    link: `${BASE_URL}/WestLuckyDraw/ExternalShareGuide?address=dist&routing=${routing}&activityId=${activityId}&fromOpenid=${OpenId}&fromCustId=${CustomerId}&shareType=${shareType2}`, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
    imgUrl: ShareImage, // 分享图标
    success: function () {
      dispatch({
